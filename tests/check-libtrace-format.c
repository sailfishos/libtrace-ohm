...
#define TEST_CONTEXT "test-context"
...
TRACE_DECLARE_MODULE(formattest, TEST_MODULE,
...
setup(void)
{
    ...
    fail_unless((cid = trace_context_open(TEST_CONTEXT)) >= 0);
    fail_unless(trace_add_module(cid, &formattest) == 0);
...
teardown(void)
{
    ...
    fail_unless(trace_del_module(cid, formattest.name) == 0);
...
void
chktrace_format_tests(Suite *suite)
{
    TCase *tc;

    tc = tcase_create("trace format");
